...
قاعدة بيانات واحدة PostgreSQL:
  - Schema عام (public) يحتوي جدول المؤسسات tenants وأكواد الاشتراك subscription_codes
  - كل مؤسسة لها Schema مستقلة داخل نفس قاعدة البيانات باسم tenant_<slug>
  - بيانات الاتصال تُقرأ حصراً من متغير البيئة DATABASE_URL
"""
...
# لا تأثير على الإنتاج (Railway وغيره)، لأنها تُعيَّن هناك كمتغيرات بيئة حقيقية بالفعل.
try:
...
# ══════════════════════════════════════════════════════════════
#  الاتصال بقاعدة البيانات (PostgreSQL فقط عبر DATABASE_URL)
# ══════════════════════════════════════════════════════════════

def _normalize_db_url(url: str) -> str:
    """Railway / Heroku أحياناً تعطي رابطاً بصيغة postgres:// القديمة،
    و SQLAlchemy يحتاج postgresql:// . هذه الدالة توحّد الصيغة فقط،
    ولا تُستخدم كأي شكل من أشكال مسار محلي."""
...
if not DATABASE_URL:
    # قيمة افتراضية للتطوير المحلي فقط، حتى يقلع السيرفر ولا يتعطل الاستيراد.
    # في أي بيئة حقيقية (Railway مثلاً) يجب ضبط DATABASE_URL دائماً.
    ...
    print(
        ...
        "postgresql://postgres:postgres@localhost:5432/studentsdb\n"
        "تأكد من وجود PostgreSQL شغال على هذا العنوان، أو اضبط DATABASE_URL "
        "في ملف .env أو كمتغير بيئة حقيقي قبل النشر."
    )
...
# ══════════════════════════════════════════════════════════════
#  قاعدة البيانات الرئيسية (Schema: public) — المستخدمون/المؤسسات/الإعدادات العامة
#  لا يوجد أي مسار محلي: كل شيء عبر DATABASE_URL على PostgreSQL
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  محرك PostgreSQL المشترك + جلسات المؤسسات (Schema-per-tenant)
#  بدلاً من ملف SQLite مستقل لكل مؤسسة، تُستخدم Schema مستقلة
#  باسم tenant_<slug> داخل نفس قاعدة بيانات PostgreSQL الواحدة.
# ══════════════════════════════════════════════════════════════
...
_TenantSessionFactory = scoped_session(sessionmaker(bind=engine))


def tenant_schema_name(slug):
    """اسم Schema آمن للمؤسسة داخل PostgreSQL: tenant_<slug>."""
    safe = re.sub(r'[^a-z0-9_]', '', (slug or '').strip().lower())
    return f'tenant_{safe}'


def get_tenant_session(slug):
    """يرجع جلسة SQLAlchemy مضبوطة على Schema المؤسسة الحالية (search_path)."""
    schema = tenant_schema_name(slug)
    sess = _TenantSessionFactory()
    sess.execute(text(f'SET search_path TO "{schema}", public'))
    return sess

# ══════════════════════════════════════════════════════════════
#  إنشاء جداول المؤسسة الجديدة (داخل Schema مستقلة بها)
# ══════════════════════════════════════════════════════════════
...
def create_tenant_schema_and_tables(slug):
    """ينشئ Schema المؤسسة (إن لم توجد) وكل جداولها الأساسية. آمن للتكرار."""
    schema = tenant_schema_name(slug)
...
def create_tenant_db(slug, org_name, owner_name, owner_email, owner_password):
    """ينشئ Schema جديدة للمؤسسة داخل PostgreSQL ويضع البيانات الأساسية،
    بدلاً من إنشاء ملف SQLite منفصل."""
    schema = tenant_schema_name(slug)
    with engine.begin() as conn:
        conn.execute(text(f'CREATE SCHEMA IF NOT EXISTS "{schema}"'))
        conn.execute(text(f'SET search_path TO "{schema}", public'))
        ...
        # إعدادات المؤسسة
        conn.execute(text(
            "INSERT INTO system_settings (system_name,system_subtitle,school_name,updated_at) "
...
def t_insert_returning_id(sql, params=None):
    """لتنفيذ INSERT مع إرجاع الـ id الجديد مباشرة (بديل last_insert_rowid في PostgreSQL).
    يجب أن تنتهي جملة الـ SQL المُمررة بدون RETURNING؛ تتم إضافتها هنا تلقائياً."""
    sql = sql.strip().rstrip(';')
    if 'RETURNING' not in sql.upper():
        sql += ' RETURNING id'
    new_id = tdb().execute(text(sql), params or {}).scalar()
    tdb().commit()
    return new_id
...
def close_tenant_session(exception=None):
    ...
    ونظّف الـ scoped_session registry حتى لا يُعاد استخدام search_path مؤسسة سابقة."""
    tenant_sess = g.pop('tenant_session', None)
    if tenant_sess is not None:
        if exception:
            tenant_sess.rollback()
        else:
            try:
                tenant_sess.commit()
            except Exception:
...
def developer_delete_tenant(slug):
    ...
    # حذف Schema المؤسسة بالكامل (بكل جداولها وبياناتها) من PostgreSQL
    schema = tenant_schema_name(slug)
    try:
        with engine.begin() as conn:
            conn.execute(text(f'DROP SCHEMA IF EXISTS "{schema}" CASCADE'))
    except Exception as e:
        print(f'[developer_delete_tenant] فشل حذف Schema {schema}: {e}')
...
def init_app():
    """Migration تلقائية بالكامل على PostgreSQL:
    1) إنشاء جداول القاعدة الرئيسية (public schema) إن لم توجد.
    2) ترقية أعمدة القاعدة الرئيسية بأمان (IF NOT EXISTS مدعومة أصلاً في PostgreSQL).
    3) إنشاء/تحديث Schema كل مؤسسة موجودة وجميع جداولها.
    لا تُمنع عملية إقلاع السيرفر بسبب فشل مؤقت في الاتصال بقاعدة البيانات
    (مثال: PostgreSQL لسه مش شغال، أو DATABASE_URL غلط)؛ بدلاً من ذلك تُطبع
    رسالة واضحة، ويمكن إعادة المحاولة بإعادة تشغيل السيرفر بعد تصحيح الاتصال.
    """
    try:
        with app.app_context():
            main_db.create_all()
            # ترقية أعمدة جدول tenants (آمنة تماماً وتُنفَّذ في كل إقلاع دون تكرار أخطاء)
            try:
                with main_db.engine.begin() as conn:
                    conn.execute(text('ALTER TABLE tenants ADD COLUMN IF NOT EXISTS owner_phone VARCHAR(20)'))
                    conn.execute(text('ALTER TABLE tenants ADD COLUMN IF NOT EXISTS subscription_until TIMESTAMP'))
            except Exception as e:
                print(f'[migration] تنبيه أثناء ترقية جدول tenants: {e}')

            # إنشاء/تحديث Schema كل مؤسسة موجودة وكل جداولها (بدون المساس بالبيانات الحالية)
            try:
            ...
            for t in tenants:
                try:
                    create_tenant_schema_and_tables(t.slug)
                except Exception as e:
                    print(f'[migration] فشل تحديث Schema المؤسسة {t.slug}: {e}')
